  ...
  for (const [name, mod] of Object.entries(window.jSquashModules)) {
    const adapter = {
      ...
      hasEncode:
        ...
        (mod.default && typeof mod.default.encode === 'function'),
      hasEncodeImage:
        typeof mod.encodeImage === 'function' ||
        (mod.default && typeof mod.default.encodeImage === 'function'),
...
function refreshModuleStatus() {
  ...
  Array.from(allKeys)
    ...
    .forEach(k => {
      ...
      loadedCell.textContent = mod ? 'yes' : 'no'
...
async function encodeWithModule(outputType, imageData, opts) {
  ...
  if (mod.default && typeof mod.default.encode === 'function')
    return await mod.default.encode(imageData, opts)
...
async function useOriginalParams() {
  ...
  if (!file && !url) {
    setStatus('请先选择本地文件或输入远程 URL 再使用原视频参数')
    return
  }
...
async function convert() {
  ...
  let moduleAvailable =
    (window.jSquashModules && window.jSquashModules[desiredFormat]) ||
    (window.jSquashModule &&
      (typeof window.jSquashModule.encode === 'function' || window.jSquashModule[desiredFormat]))
  if (!moduleAvailable) {
    ...
    try {
      ...
      moduleAvailable =
        (window.jSquashModules && window.jSquashModules[desiredFormat]) ||
        (window.jSquashModule &&
          (typeof window.jSquashModule.encode === 'function' ||
            window.jSquashModule[desiredFormat]))
    } catch (e) {
      console.warn('动态加载默认 jSquash 模块失败：', e.message || e)
    }
    if (!moduleAvailable) {
      setStatus(
        '编码器未加载：' + desiredFormat + '。请点击“刷新模块状态”或确保网络/CDN 可用后重试。'
      )
      $('convertBtn').disabled = false
      return
    }
  ...
  setStatus(
    `视频就绪：时长 ${video.duration.toFixed(2)}s，采样 ${fps} fps，输出 ${targetW}x${targetH}`
  )
  ...
  if (frames.length > 1 && !supportsMultiFrame(desiredFormat)) {
    setStatus(
      '所选格式或其已加载模块不支持多帧动画：' +
        desiredFormat +
        '。请选择支持动画的格式（例如 WebP/AVIF）或点击“刷新模块状态”加载支持的模块。'
    )
  ...
  try {
    ...
    const out = await encodeWithJsquash(frames, format, {
      quality: parseInt($('quality').value || '1', 10)
    })
...
async function encodeWithJsquash(frames, format, opts) {
  ...
  const frameBuffers = frames.map(f =>
    f.data instanceof Uint8Array ? f.data : new Uint8Array(f.data.buffer || f.data)
  )
  ...
  if (window.jSquashModules && window.jSquashModules[format]) {
    ...
    if (adapter.hasEncodeFrames) {
      try {
        return await mod.encodeFrames(frameBuffers, {
          width,
          height,
          delays,
        ...
        log('adapter encodeFrames failed: ' + e.message)
      }
    }
    // 如果模块被探测为可编码多帧（heuristic），尝试使用 encode with frames
    if (adapter.encodesMultiple && typeof mod.encode === 'function') {
      try {
        return await mod.encode(frameBuffers, { width, height, delays, quality: opts.quality })
      } catch (e) {
        log('adapter encode(frames) failed: ' + e.message)
      }
    }
    // 退回到单帧 encode：尝试多种常见调用签名以兼容不同模块实现
    if (adapter.hasEncode) {
      const encoder =
        typeof mod.encode === 'function' ? mod.encode : mod.default && mod.default.encode
      if (encoder) {
        const tryCalls = [
          // raw Uint8Array + opts
          ...
          // ArrayBuffer view
          async () =>
            encoder(frameBuffers[0].buffer || frameBuffers[0], {
              width,
              height,
              quality: opts.quality
            }),
          ...
          async () =>
            encoder({ width, height, frames: frameBuffers, delays }, { quality: opts.quality })
        ]
        for (const fn of tryCalls) {
          try {
            ...
            log(
              'adapter single-frame encode attempt failed: ' +
                (e && e.message) +
                ' — trying next signature'
            )
          }
        }
  ...
  if (!js)
    throw new Error(
      'Global jSquash bundle not loaded; per-format module attempts either failed or are not supported. See log for details.'
    )

  // 一：通用 encode(frames, format, opts)
  if (typeof js.encode === 'function') {
    try {
      return await js.encode({ width, height, frames: frameBuffers, delays }, format, opts)
    } catch (e) {
      log('js.encode failed: ' + e.message)
    }
  }
  ...
  if (js[format] && typeof js[format].encode === 'function') {
    try {
      return await js[format].encode({ width, height, frames: frameBuffers, delays }, opts)
    } catch (e) {
      log('js[format].encode failed: ' + e.message)
    }
  }
  ...
  if (typeof js.encodeImage === 'function') {
    try {
      return await js.encodeImage(frameBuffers[0], { width, height, format, quality: opts.quality })
    } catch (e) {
      log('js.encodeImage failed: ' + e.message)
    }
  }
  ...
  if (typeof js.encodeFrames === 'function') {
    try {
      return await js.encodeFrames(frameBuffers, {
        width,
        height,
        delays,
        format,
      ...
      log('js.encodeFrames failed: ' + e.message)
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('fileInput').addEventListener('change', e => {
    if (e.target.files && e.target.files.length) $('convertBtn').disabled = false
  })
