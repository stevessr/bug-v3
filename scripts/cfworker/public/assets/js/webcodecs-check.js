...
function pill(ok) {
  return `<span style="padding:2px 6px;border-radius:10px;background:${ok ? '#d1fae5' : '#fee2e2'};color:${ok ? '#065f46' : '#991b1b'};font-size:12px;">${ok ? '支持' : '不支持'}</span>`
}
...
async function detect() {
  ...
  const hasWebCodecsNS =
    'VideoDecoder' in window ||
    'VideoEncoder' in window ||
    'ImageDecoder' in window ||
    'ImageEncoder' in window ||
  ...
  async function testImageSupport(mime) {
    if (!hasImageEncoder || typeof ImageEncoder.isConfigSupported !== 'function')
      return { supported: false, error: 'ImageEncoder 或 isConfigSupported 不可用' }
    try {
  ...
  async function testVideoCodec(codec) {
    if (!('VideoEncoder' in window) || typeof VideoEncoder.isConfigSupported !== 'function')
      return { supported: false, error: 'VideoEncoder.isConfigSupported 不可用' }
    try {
  ...
  async function testAudioCodec(codec) {
    if (!('AudioEncoder' in window) || typeof AudioEncoder.isConfigSupported !== 'function')
      return { supported: false, error: 'AudioEncoder.isConfigSupported 不可用' }
    try {
  ...
  function block(title, rows) {
    const list = rows
      .map(
        r =>
          `<tr><td>${r.name}</td><td>${pill(r.ok)}</td><td><pre style="white-space:pre-wrap;word-break:break-all;">${r.note || ''}</pre></td></tr>`
      )
      .join('')
    return `
  ...
  detailsEl.innerHTML = [
    block('Image 编码能力', [
      {
        name: 'AVIF (ImageEncoder)',
        ok: !!imgAvif.supported,
        note: imgAvif.supported ? JSON.stringify(imgAvif.config, null, 2) : imgAvif.error || ''
      },
      {
        name: 'WebP (ImageEncoder)',
        ok: !!imgWebp.supported,
        note: imgWebp.supported ? JSON.stringify(imgWebp.config, null, 2) : imgWebp.error || ''
      },
      {
        name: 'Canvas toDataURL WebP',
        ok: canWebP,
        note: canWebP ? 'toDataURL("image/webp") 支持' : '不支持 WebP 数据导出'
      }
    ]),
    block('Video 编码能力', [
      {
        name: 'AV1 (av01)',
        ok: !!vAv1.supported,
        note: vAv1.supported ? JSON.stringify(vAv1.config, null, 2) : vAv1.error || ''
      },
      {
        name: 'VP9 (vp09)',
        ok: !!vVp9.supported,
        note: vVp9.supported ? JSON.stringify(vVp9.config, null, 2) : vVp9.error || ''
      },
      {
        name: 'H.264 (avc1)',
        ok: !!vH264.supported,
        note: vH264.supported ? JSON.stringify(vH264.config, null, 2) : vH264.error || ''
      }
    ]),
    block('Audio 编码能力', [
      {
        name: 'Opus',
        ok: !!aOpus.supported,
        note: aOpus.supported ? JSON.stringify(aOpus.config, null, 2) : aOpus.error || ''
      },
      {
        name: 'AAC (mp4a.40.2)',
        ok: !!aAac.supported,
        note: aAac.supported ? JSON.stringify(aAac.config, null, 2) : aAac.error || ''
      }
    ])
  ].join('')
}
