...
if (!extensionId || !hostPath) {
  console.error(
    'Usage: node scripts/mcp-bridge/create-host-manifest.js --extension-id <id> --host-path <abs path> [--output <path>]'
  )
  process.exit(1)
}
...
const payload = template.replace('__EXTENSION_ID__', extensionId).replace('__HOST_PATH__', hostPath)
