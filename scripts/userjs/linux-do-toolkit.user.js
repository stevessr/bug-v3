...
;(function () {
  ...
  const ReactionHelper = {
    ...
    async runUserReaction() {
      ...
      for (let i = 0; i < this.targetPostIds.length; i++) {
        ...
        const result = await this.sendReactionToPost(
          item.id,
          i + 1,
          this.targetPostIds.length,
          'user'
        )
    ...
    async addGroupMembers() {
      ...
      if (
        !confirm(
          `确定要将 ${usernameList.length} 个用户添加到群组 ${groupId} 吗？\n\n用户列表：\n${usernameList.join('\n')}`
        )
      ) {
        return
      }
    ...
    init() {
      ...
      toggleBtn.addEventListener(
        ...
        e => {
          const touch =
            Array.from(e.changedTouches || []).find(t => t.identifier === clickStartTouchId) ||
            e.changedTouches[0]
          if (!touch) return
          const clickDuration = Date.now() - clickStartTime
          const moveDistance = Math.sqrt(
            Math.pow(touch.clientX - clickStartX, 2) + Math.pow(touch.clientY - clickStartY, 2)
          )
      ...
      // 生成表情选择器选项
      const reactionOptions = this.REACTIONS.map(
  ...
  const LikeCounter = {
      ...
      window.fetch = async function (...args) {
        ...
        if (url && (url.includes('/toggle.json') || url.includes('/custom-reactions/'))) {
          response
            .clone()
            .json()
            .then(data => self.processToggleResponse(url, data))
            .catch(() => {})
        }
        return response
      }
      ...
      window.XMLHttpRequest.prototype.send = function () {
        ...
        if (url && (url.includes('/toggle.json') || url.includes('/custom-reactions/'))) {
          this.addEventListener('load', function () {
            try {
              self.processToggleResponse(url, JSON.parse(this.responseText))
            } catch (e) {}
          })
        }
    ...
    updateUI() {
      ...
      const dailyLimit =
        (this.currentUser && this.CONFIG.LIMITS[this.currentUser.trust_level]) || 50
      ...
      if (isCooldown) {
        ...
        const h = Math.floor(diff / 3600000)
        this.cooldownTicker = setTimeout(
          () => this.requestUiUpdate(true),
          h === 0 ? 1000 : 30 * 1000
        )
      }
      ...
      while (keepFetching && pages < 5) {
        try {
          const res = await this.fetchWithTimeout(
            `${this.CONFIG.HOST}/user_actions.json?limit=${limit}&username=${username}&filter=1&offset=${offset}`
          )
    ...
    async syncRemote() {
      ...
      if (!this.currentUser) {
        try {
          this.currentUser = require('discourse/models/user').default.current()
        } catch (e) {}
      ...
      try {
        const [likes, reactions] = await Promise.all([
          this.fetchUserActions(username),
          this.fetchReactions(username)
        ])
        ...
        if (savedCooldown > Date.now()) {
          ...
          placeholders = this.state.timestamps.filter(
            ts => ts >= expectedBase && ts < expectedBase + 5000
          )
        }

        this.state.timestamps = Array.from(
          new Set([...dedupedTimestamps, ...localNewer, ...placeholders])
        )
        this.state.lastSync = Date.now()
        const limit = this.CONFIG.LIMITS[this.currentUser.trust_level] || 50
        this.state.matched = savedMatched
        ...
        for (const m of mutations) {
          if (m.addedNodes.length) {
            for (const node of m.addedNodes) {
              if (
                node.nodeType === 1 &&
                (node.classList.contains('discourse-reactions-picker') ||
                  node.querySelector('.discourse-reactions-picker'))
              ) {
