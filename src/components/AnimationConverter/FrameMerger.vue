...
<template>
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
    ...
    <p class="text-gray-600 mb-6">将多个图片帧合并为动图</p>
    ...
    >
      ...
      <div v-if="selectedFiles.length === 0">
        ...
        <p class="text-xs text-gray-500 mt-1">支持 PNG, JPG, WebP 等图片格式，可选择多个文件</p>
      </div>
    ...
    <div v-if="selectedFiles.length > 0" class="mt-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <button @click="sortFrames" class="text-sm text-blue-600 hover:text-blue-500">
          按名称排序
        </button>
      </div>
    ...
    <div v-if="selectedFiles.length > 0" class="mt-6 space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">循环次数</label>
          <select
            v-model="loopCount"
            class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
          >
