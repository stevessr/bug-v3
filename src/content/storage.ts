    ...
    if (resp && resp.success && resp.data) {
      ...
      if (Array.isArray(groups) && groups.length > 0) {
        ...
        if (validGroups > 0 && totalEmojis > 0) {
          ...
          console.log(`[Emoji Extension] Successfully loaded ${validGroups} valid groups with ${totalEmojis} total emojis (from background)`);
        } else {
          console.warn('[Emoji Extension] Groups exist but contain no valid emojis, using defaults (from background)');
          cachedState.emojiGroups = [];
        }
      } else {
        console.warn('[Emoji Extension] No valid emoji groups found in background response, using defaults');
        cachedState.emojiGroups = [];
      }
