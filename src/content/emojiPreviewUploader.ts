...
class EmojiPreviewUploader {
  ...
  private async performUpload(file: File): Promise<EmojiUploadResponse> {
    ...
    // Get CSRF token and cookies - this is where we'll request from options page if needed
    const { csrfToken, cookies } = await this.getAuthInfo()
  ...
  private async getAuthInfo(): Promise<{ csrfToken: string; cookies: string }> {
    // Try to get CSRF token and cookies locally first
    let csrfToken = this.getCSRFToken()
    let cookies = document.cookie

    // If we don't have auth info and we're not on linux.do, request from options page
    if ((!csrfToken || !cookies) && !window.location.hostname.includes('linux.do')) {
      try {
        const authInfo = await this.requestAuthFromOptions()
        csrfToken = authInfo.csrfToken || csrfToken
        cookies = authInfo.cookies || cookies
      } catch (error) {
        logger.warn('[Emoji Preview Uploader] Failed to get auth info from options:', error)
      }
    }
  ...
  private async requestAuthFromOptions(): Promise<{ csrfToken: string; cookies: string }> {
    // Send message to options page to request cookies and CSRF token from linux.do
    return new Promise((resolve, reject) => {
      if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.sendMessage) {
        chrome.runtime.sendMessage(
          {
            type: 'REQUEST_LINUX_DO_AUTH',
            action: 'getLinuxDoAuth'
          },
          response => {
            if (chrome.runtime.lastError) {
              ...
              reject(new Error('Failed to get auth info from options'))
            }
          }
        )
      } else {
