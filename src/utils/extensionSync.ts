 ...
 * Handles multiple sync targets specifically for browser extensions
 */
...
export interface WebDAVConfig extends SyncConfigBase {
  type: 'webdav'
  url: string
  username: string
  password: string
...
export interface S3Config extends SyncConfigBase {
  type: 's3'
  endpoint: string
  region: string
  bucket: string
...
export interface CloudflareConfig extends SyncConfigBase {
  type: 'cloudflare'
  url: string
  authToken: string
...
export type SyncConfig = WebDAVConfig | S3Config | CloudflareConfig
...
export class S3SyncTarget implements ISyncTarget {
  ...
  private async signS3Request(
    ...
    // For a production implementation, consider using a dedicated library like aws-sdk
    ...
    // Create headers with required AWS elements
    const headers: Record<string, string> = {
      'x-amz-date': timeStamp,
      'x-amz-content-sha256': body ? this.sha256Hash(body) : this.sha256Hash(''),
      Authorization: 'Temporary' // This will be replaced after calculating the signature
    }
    ...
    // For this simplified implementation, we'll return basic authorization headers
    // A full implementation would require calculating the signature
    return {
      ...headers,
      Authorization: `AWS4-HMAC-SHA256 Credential=${accessKeyId}/${dateStamp}/${region}/${service}/aws4_request, SignedHeaders=host;x-amz-date;x-amz-content-sha256, Signature=UNSIGNED`
    }
  }

  private sha256Hash(str: string): string {
    // For a real implementation, we'd need to properly hash the content
  ...
  async test(): Promise<SyncResult> {
    try {
      ...
      // This is a simplified approach - a full implementation would use proper S3 API calls
      const headers = await this.signS3Request('GET', url, null, this.config.region, 's3')

      const response = await fetch(url, {
        method: 'HEAD',
        headers: {
          ...headers,
          // For testing with services that don't require full AWS signature
          Authorization: `Basic ${btoa(`${this.config.accessKeyId}:${this.config.secretAccessKey}`)}`
        }
      })

      // Accept 200 (exists), 404 (doesn't exist but accessible), or 403 (exists but no read permission)
  ...
  async push(data: SyncData, onProgress?: ProgressCallback): Promise<SyncResult> {
    try {
      ...
      // For this implementation, we'll use a direct PUT request with basic Auth
      // A production implementation would need proper AWS Signature V4
      const response = await fetch(url, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json; charset=utf-8',
          Authorization: `AWS4-HMAC-SHA256 Basic ${btoa(`${this.config.accessKeyId}:${this.config.secretAccessKey}`)}`
        },
        body
      })
  ...
  async pull(
    ...
    try {
      ...
      // For this implementation, we'll use a direct GET request with basic Auth
      // A production implementation would need proper AWS Signature V4
      const response = await fetch(url, {
        method: 'GET',
        headers: {
      ...
      if (response.status === 404) {
        return {
          success: false,
          message: 'No data found on S3 bucket',
...
export class CloudflareSyncTarget implements ISyncTarget {
  ...
  async push(data: SyncData, onProgress?: ProgressCallback): Promise<SyncResult> {
    try {
      ...
      // Prepare data to upload - split into multiple chunks for Cloudflare Workers KV
      const itemsToPush = [
    ...
    try {
      ...
      for (const item of pulledItems) {
        if (item.key === 'settings') {
          ...
          // Decode the key to get the original group name
          try {
            const groupName = decodeURIComponent(item.key)
            pulledData.emojiGroups!.push({ ...item.data, name: groupName })
          } catch {
            // If decoding fails, use the key as the name
            pulledData.emojiGroups!.push({ ...item.data, name: item.key })
          }
        }
      }

      // Try to get top level version/timestamp if it was set
      if (pulledData.settings && (pulledData.settings as any).version)
        version = (pulledData.settings as any).version
      if (pulledData.settings && (pulledData.settings as any).timestamp)
        timestamp = (pulledData.settings as any).timestamp

      const finalData: SyncData = {
        settings: pulledData.settings || ({} as AppSettings),
...
export function createSyncTarget(config: SyncConfig): ISyncTarget {
  switch (config.type) {
    case 'webdav':
