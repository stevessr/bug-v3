...
/**
 * 加载并解压缩 brotli 压缩的默认表情组数据
 * 破坏性更新：只支持压缩版本，不提供降级方案
 */
export async function loadCompressedDefaultGroups(): Promise<EmojiGroup[]> {
  // 检查浏览器支持
  if (!isBrotliSupported()) {
    throw new Error('❌ 浏览器不支持 Brotli 解压缩，无法加载配置数据')
  }

  const compressedUrl = '/assets/defaultEmojiGroups.json.br'
  ...
  try {
    ...
    console.log(`✅ 成功加载压缩配置：${data.groups.length} 个表情组`)
    return data.groups
    
  } catch (error) {
    if (error instanceof Error) {
      throw new Error(`❌ 加载压缩配置失败: ${error.message}`)
    }
    throw new Error('❌ 加载压缩配置时发生未知错误')
  }
}
