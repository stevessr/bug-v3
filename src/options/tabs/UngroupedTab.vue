    ...
    <div v-else>
      <div style="display:flex; gap:8px; margin-bottom:8px; align-items:center">
        <a-select v-model:value="targetGroup" style="min-width:220px" placeholder="选择目标分组">
          <a-select-option v-for="g in groups" :key="g.UUID" :value="g.UUID">{{ g.displayName }} ({{ g.UUID }})</a-select-option>
        </a-select>
        <a-button type="primary" :disabled="!targetGroup || selected.length===0" @click="moveSelected">移动到分组</a-button>
        <a-button @click="selectAll">全选</a-button>
        <a-button @click="clearSelection">清空</a-button>
      </div>
      <a-list bordered>
        <a-list-item v-for="e in items" :key="e.UUID">
          <a-space style="width:100%; display:flex; justify-content:space-between; align-items:center">
            <div style="display:flex; gap:8px; align-items:center">
              <a-checkbox v-model:checked="selectedMap[e.UUID]" @change="onCheck(e)" />
              <img :src="e.displayUrl || e.realUrl" style="width:40px; height:40px; object-fit:cover; border-radius:4px" />
              <div>
                <div>{{ e.displayName || '（无名）' }}</div>
                <div style="font-size:12px; color:var(--ant-text-color-secondary)">{{ e.UUID }}</div>
              </div>
            </div>
            <div>
...
export default defineComponent({
  setup() {
    ...
    const selected = computed(() => Object.keys(selectedMap.value).filter((k) => selectedMap.value[k]))
    function load() {
  ...
  onMounted(load)
  return { items, groups, selectedMap, targetGroup, selected, selectAll, clearSelection, onCheck, moveSelected, moveSingle }
  },
})
</script>
