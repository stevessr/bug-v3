...
const parseWalineConfig = (data: any): Array<{ name: string; url: string }> => {
  ...
  try {
    ...
    if (typeof data === 'object' && data !== null) {
      ...
      if (Array.isArray((data as any).items) && (data as any).prefix && (data as any).type) {
        const prefix = (data as any).prefix || ''
        const type = (data as any).type || 'png'
        const baseUrl = (data as any).baseUrl || 'https://cdn.jsdelivr.net/gh/walinejs/emojis/'

        for (const item of (data as any).items) {
          if (typeof item === 'string') {
            const name = item
            const url = `${baseUrl}${(data as any).name?.toLowerCase() || 'weibo'}/${prefix}${item}.${type}`
      ...
      // Format 1: { "category": { "type": "image", "container": [...] } }
        ...
        for (const categoryKey in data) {
          const category = (data as any)[categoryKey]
