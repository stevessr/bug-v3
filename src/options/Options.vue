    ...
    <a-layout>
      ...
      <a-layout-content style="margin: 24px 16px; padding: 24px; background: #f0f2f5">
        <router-view
          ...
          @update:imageScale="(value: number) => emojiStore.updateSettings({ imageScale: value })"
          @update:showSearchBar="(checked: boolean) => emojiStore.updateSettings({ showSearchBar: checked })"
          @update:outputFormat="(value: string) => emojiStore.updateSettings({ outputFormat: value as 'markdown' | 'html' })"
          @update:forceMobileMode="(checked: boolean) => emojiStore.updateSettings({ forceMobileMode: checked } as any)"
