    ...
    <div class="bg-white rounded-lg p-6 w-full max-w-md" @click.stop>
      ...
      <div class="space-y-4">
        <div>
          ...
          <input v-model="name" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="输入分组名称" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">分组图标 / 图片链接</label>
          <input v-model="icon" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="例如：😀 或 https://..." />
          <div v-if="isImageUrl(icon)" class="mt-2 text-center">
            <img :src="icon" alt="预览" class="w-10 h-10 object-contain mx-auto border border-gray-200 rounded" @error="handleImageError" />
          </div>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">分组颜色</label>
          <div class="flex gap-2">
            <div v-for="color in colorOptions" :key="color" :class="['w-8 h-8 rounded cursor-pointer border-2', selectedColor === color ? 'border-gray-900' : 'border-gray-300']" :style="{ backgroundColor: color }" @click="selectedColor = color"></div>
          </div>
        </div>
      </div>
      <div class="flex justify-end gap-3 mt-6">
        <button @click="close" class="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded transition-colors">取消</button>
        <button @click="create" class="px-4 py-2 text-sm bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors">创建</button>
      </div>
    </div>
  </div>
</template>
