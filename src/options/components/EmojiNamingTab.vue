  ...
  <div v-if="activeTab === 'emoji-naming'" class="space-y-6">
      ...
      <div class="border-t pt-6">
        ...
        <div v-if="selectedAIProvider === 'openai-compatible'" class="space-y-4">
          ...
          <div>
            ...
            <a-input 
              v-model:value="aiConfigs.openaiCompatible.model" 
        ...
        <div v-if="selectedAIProvider === 'chrome-ai'" class="space-y-4">
          ...
          <div class="p-4 bg-blue-50 rounded-lg">
            <div class="flex items-center space-x-2 mb-2">
              <div class="w-3 h-3 rounded-full" :class="browserAIStatus.chrome ? 'bg-green-500' : 'bg-red-500'"></div>
        ...
        <div v-if="selectedAIProvider === 'edge-ai'" class="space-y-4">
          ...
          <div class="p-4 bg-blue-50 rounded-lg">
            <div class="flex items-center space-x-2 mb-2">
              <div class="w-3 h-3 rounded-full" :class="browserAIStatus.edge ? 'bg-green-500' : 'bg-red-500'"></div>
              <span class="font-medium">Edge AI 状态</span>
            </div>
            <p class="text-sm text-gray-600">
              使用 Microsoft Edge AI 写作辅助 API 进行图像理解。
            </p>
          </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      <div class="mb-6">
        ...
        <div class="grid grid-cols-8 md:grid-cols-12 lg:grid-cols-16 gap-2 max-h-64 overflow-y-auto border rounded p-4">
          <div 
            v-for="emoji in availableEmojis" 
            :key="emoji.id"
            :class="selectedEmojis.has(emoji.id) ? 'border-2 border-blue-500 bg-blue-50' : 'border border-gray-200'"
      ...
      <div class="space-y-4">
        <div>
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-sm text-gray-500">{{ processingStatus.current }}/{{ processingStatus.total }}</span>
          </div>
          <a-progress :percent="Math.round((processingStatus.current / processingStatus.total) * 100)" />
        </div>
        
        <div class="text-sm text-gray-600">
          当前: {{ processingStatus.currentEmoji }}
        </div>
        
        <div class="text-sm">
          状态: {{ processingStatus.message }}
        </div>
      </div>
    </div>
    ...
    <div v-if="renamingResults.length > 0" class="bg-white rounded-lg shadow-md p-6">
          ...
          <div class="flex items-start space-x-4">
            ...
            <div class="flex-1">
              ...
              <div class="mt-3 flex space-x-2">
                ...
                <a-button size="small" @click="skipRename(result.emojiId)">
                  跳过
                </a-button>
              </div>
            </div>
      ...
      <div class="mt-6 flex space-x-4">
        <a-button type="primary" @click="applyAllRenames">
          应用所有重命名
        </a-button>
...
const namingPrompt = ref('分析这个表情包图像，为它提供 3-5 个简洁、准确的中文名称建议，名称应该描述图像中的动作、表情或情绪。每个建议用换行分隔。')
...
const isAIConfigured = computed(() => {
  switch (selectedAIProvider.value) {
    ...
    case 'openai-compatible':
      return aiConfigs.value.openaiCompatible.endpoint && 
