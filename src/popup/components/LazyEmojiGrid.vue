...
<template>
  ...
  <div v-show="isActive" class="lazy-emoji-grid-wrapper">
    ...
    <div v-else class="emoji-grid-empty">
      <svg class="emoji-grid-empty-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path
      ...
      <a-button v-if="showAddButton" @click="$emit('openOptions')" class="emoji-grid-empty-btn">
        {{ t('goAddEmojis') }}
      </a-button>
    </div>
  </div>
