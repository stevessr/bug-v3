...
function isPixivPage(): boolean {
  try {
    ...
    const twitterMeta = document.querySelector('meta[property="twitter:site"]') ||
    ...
    if (ogImage.includes('pixiv.net') || ogImage.includes('pximg.net') || ogImage.includes('embed.pixiv.net')) {
      return true;
    }
...
async function tryGetImageViaCanvas(url: string): Promise<{ success: true; blob: Blob } | { success: false; error: any }> {
...
async function sendEmojiToBackground(blob: Blob, emojiName: string, filename: string, originUrl?: string) {
  try {
    const chromeAPI = (window as any).chrome;
