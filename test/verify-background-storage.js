...
async function simulateGetEmojiDataHandler(mockData) {
  ...
  try {
    ...
    if (freshData && freshData.emojiGroups) {
      ...
      const totalEmojis = response.data.groups.reduce((sum, group) => sum + (group.emojis?.length || 0), 0)
...
async function runTests() {
  ...
  const mockDataWithEmojis = {
    ...
    'emojiGroups-表情-uuid': {
      ...
      order: 2,
      emojis: [
        { UUID: 'emoji-6', displayName: '爱心', url: 'https://example.com/heart.png' }
      ]
    }
  }
  ...
  if (!result2.success || result2.data?.groups?.length === 0) {
    ...
    console.log('❌ Test 2 FAILED: Expected empty result, got', result2.data?.groups?.length || 0, 'groups\n')
    return false
  }
  ...
  if (result3.success && Object.keys(result3.data.settings).length === 3 && result3.data.groups.length === 0) {
...
runTests().then(success => {
  if (success) {
    console.log('🎉 All tests passed! Background script correctly loads emoji data from extension storage.')
